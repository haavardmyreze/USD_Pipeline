...
husk --renderer Karma --engine xpu \
     ...
     $SHOTS/neon/0010/usd/neon-0010.usda
```
...
def Xform "CharRobot" (
    instanceable = true
    references = @$ASSETS/char-robot/usd/char-robot.usda@
)
```
...
Assets, sets, and shots all share the same shape: a `blocks/` folder holding one folder per block, an `assembly/` folder holding the working HIP that composes them, and a top-level `usd/` folder holding the published assembly (the shot's assembly is its shot root).
...
```
$ASSETS/char-robot/usd/char-robot.usda
$SETS/living-room/usd/set-living-room.usda
$SHOTS/neon/0010/blocks/anim/usd/neon-0010_anim.usda
$LIBRARY/materials/metal-bare.usda
```
